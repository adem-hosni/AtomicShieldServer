...
export function useSellAuthEmbed(): SellAuthEmbedHook {
  ...
  const checkout = useCallback(async ({ cart, shopId, modal = true, scrollTop = true }: CheckoutOptions) => {
    ...
    if (!altchaToken) {
      console.warn('Captcha token not available, retrying...');
      // Try to trigger captcha refresh
      setAltchaKey(prev => prev + 1);
      throw new Error('Captcha verification required. Please try again in a moment.');
    }

    setIsLoading(true);

    const requestData = { cart, shopId, altcha: altchaToken };
    console.log('SellAuth API Request:', requestData);
    ...
    try {
      ...
      console.log('SellAuth API Response Status:', response.status);
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('SellAuth API Error Response:', errorText);
